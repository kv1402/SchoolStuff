...
/**
 * The main class of the P3 exercise. Has references to all parts of the system
 * and contains the main control loop and event processing methods.
 */
public class Simulator implements Constants
{
    ...
    public void simulate() {
        ...
        while (clock < simulationLength) {
            ...
            clock = event.getTime();
            // Let the cpu, memory, i/o unit and GUI know that time has passed
        ...
        // End the simulation by printing out some statistics
    ...
    private void flushMemoryQueue() {
        ...
        while(p != null) {
            // Prepare the process that was allocated memory for execution
            p.calculateTimeToNextIoOperation();
    ...
    private void processIoRequest() {
        // Get the active process from the CPU
        Process p = cpu.getActiveProcess();
        p.leftCpu(clock);
        // Add the process to the I/O queue
        eventQueue.insertEvent( io.addIoRequest(p, clock) );
        // Switch in a new process in the CPU
        eventQueue.insertEvent( cpu.activeProcessLeft(clock) );
    }
    ...
    private void endIoOperation() {
        Process endingProcess = io.removeActiveProcess();
        // Move the finished process to the CPU queue
        endingProcess.leftIo(clock);
        eventQueue.insertEvent( cpu.insertProcess(endingProcess, clock) );
